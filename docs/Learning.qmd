...
tidied_post_meal_data |>
  filter(BMI < 30) |>
  ggplot(aes(x = Age, y = auc_ins, colour = Group)) +
  geom_point()
```
```{r}
tidied_post_meal_data |>
...
tidied_post_meal_data |>
  filter(BMI < 30) |>
  mutate(old = if_else(Age > 40, "Old", "Young")) |>
  ggplot(aes(x = BMI, y = insulin_glucose_ratio, colour = Group)) +
  geom_point() +
  facet_grid(rows = vars(old))
```
...
tidied_post_meal_data |>
  ...
  summarise(
    ...
    "Mean Body Mass Index (kg/m^2^)" = round(mean(BMI), 1)
  ) |>
  ungroup() |>
  knitr::kable()
```
